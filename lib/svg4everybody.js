	...
	xhr.onreadystatechange = function () {
		...
		if (xhr.readyState === 4) {
			...
			xhr._embeds.splice(0).map(function (item) {
				...
				if (!target) {
					target = xhr._cachedTarget[item.id] = getElementById(cachedDocument, item.id);
				}
...
function getElementById(doc, id) {
	if (typeof doc.getElementById === 'function') {
		try {
			...
			// XXX catching errors here *might* cause problems with
			// otherwise invalid ids selectors
		}
	}
